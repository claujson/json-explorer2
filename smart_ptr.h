...
namespace wiz {
    ...
    private:
        ...
        void enter(const SmartPtr2<T>& sp)
        {
            //wizard::assertNotnullptr( sp );
            //wizard::assertnullptr( this->ptr );
        ...
        void initFromOther(const SmartPtr2<T>& sp)
        {
            //wizard::assertNotEquals( this, &sp );
           // if( this == sp.getThis() ) { return; } 
    ...
    public:
        ...
        SmartPtr2(T* ptr, const int option) // option 1,2,..
            : option(option)
        {
            // ptr <- new T(~~);
            if (ptr) {
        ...
        SmartPtr2(const SmartPtr2<T>& sp)
            : option(sp.option)
        {
            initFromOther(sp);
        }
        SmartPtr2(SmartPtr2<T>&& sp)
            :option(sp.option)
        {
            Inner* temp = this->inner;
            this->inner = sp.inner;
    ...
    public:
        ...
        SmartPtr2<T>& operator=(const SmartPtr2<T>& _sp)
        {
            // temp link
            SmartPtr2<T> tempLink(_sp);
    ...
    public:
        ...
        ///
    public:
        bool hasSameObject(const SmartPtr2<T>& wsp) const
        {
            return this->inner == wsp.inner;
        }
    };
